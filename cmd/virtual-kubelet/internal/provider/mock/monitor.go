...
func (p *MockProvider) generateNodeMetrics(metricsMap map[string][]*dto.Metric, resourceType string, label []*dto.LabelPair) map[string][]*dto.Metric {
	...
	userTime, systemTime, totalCPUTime, err := getNodeCPUTime()
	if err != nil {
...
func (p *MockProvider) generateProcessMetrics(metricsMap map[string][]*dto.Metric, resourceType string, label []*dto.LabelPair) map[string][]*dto.Metric {
	var (
		cpuMetricSuffix    = "_cpu_usage_seconds_total" // the rate of change of this metric is the cpu usage
		memoryMetricSuffix = "_memory_working_set_bytes"
		cpuDummyValue      = 0.1 // use the time since it's a monotonically increasing value
...
func getNodeCPUTime() (float64, float64, float64, error) {
...
func getNodeMemory() (uint64, error) {
...
// func getProcessMetrics(pgid) (float64, float64, float64, error) {
// 	cpuTimes, err := cpu.Times(false)
// 	if err != nil {
// 		return 0, 0, 0, err
// 	}
