...
type ResourceUpdater interface {
	...
	UpdateSecret(ctx context.Context, pod *v1.Pod, secret *v1.Secret) error
}
...
func newPodResourceWatcher(informerFactory informers.SharedInformerFactory) *watcher {
	return &watcher{
		configs:         make(map[types.NamespacedName][]*v1.Pod),
		cmKeysByPod:     make(map[types.NamespacedName][]types.NamespacedName),
		secrets:         make(map[types.NamespacedName][]*v1.Pod),
...
func (w *watcher) Watch(pod *v1.Pod) {
	if w == nil {
		return
	}
	w.mu.Lock()
...
func (w *watcher) Unwatch(pod *v1.Pod) {
	if w == nil {
		return
	}
	w.mu.Lock()
